...
use DBI;
use Getopt::Long;
GetOptions(\%opts, 'g|gff=s', 'o|output=s','i|id=s');
...
for $key(sort{$a<=>$b} keys %mRNA){
	...
	for $keya(sort{$a<=>$b} keys %{$overlap{$key}}){
		...
		if($end >= $start && $start != $keya){
			$subdomains{$key}{$start}=$end;
		}
		$start = $overlap{$key}{$keya} + 1;
	}
	$end = $start + ($cend - $start) - 1;
	if($end >= $start){
		$subdomains{$key}{$start}=$end - 1;
	}
}
...
for $key(sort {$a<=>$b} keys %subdomains){
	...
	for $keya(sort {$a <=>$b} keys %{$subdomains{$key}}){
		...
		$key =~ /AT(\d+)G(\d+)\.\d+/;
		$Chr = "Chr".$1;
		print OUT $Chr."\tintron\t".$keya."\t".$subdomains{$key}{$keya}."\t*\t".$name."\t".$key."\n";
	}
}
...
close(OUT);
close(GFF);
system("RScript order.R $opts{o}");

########### conexión con mysql
 open(GFF, "$opts{o}") || die "error i couldn't open the infile\n";
 open(OUT1, ">$opts"."annotation") || die "error i can not create the annotation file\n";
 open(OUT2, ">$opts"."subtype") || die "error i can not create the subdomain file\n";
 open(OUT3, ">$opts"."subdomain") || die "error i can not create the subtype file\n";
 open(OUT4, ">$opts"."no_match") || die "error i can not create the annotation file\n";
